...
void MovePawn::append(const QPair<uint, uint> &point, PlaceType type) {
  ...
  if (!m_first && abs((long) point.first - (long) m_x) == 2)
    m_currentList.removeAt(m_currentList.indexOf(point));

  if (type == Empty) {
    if (!m_first && abs((long) point.first - (long) m_x) == 2)
      m_currentList.removeAt(m_currentList.indexOf(point));
    if (abs((long) point.second - (long) m_y) == 1)
      m_currentList.removeAt(m_currentList.indexOf(point));
  } else if (abs((long) point.second - (long) m_y) == 0) {
    m_currentList.removeAt(m_currentList.indexOf(point));

    // we dont have to step over figures
    if (m_first && abs((long) point.first - (long) m_x) == 1) {
