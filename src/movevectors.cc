...
MoveVectors::MoveVectors(QObject *parent)
    : MovePoints(parent)
{
}
...
void MoveVectors::append(const QPair<uint, uint> &point, MovePoints::PlaceType type)
{
    ...
    while (!(posX == point.first && posY == point.second))
    {
        ...
        if(m_stops.indexOf(QPair<uint, uint>(posX, posY)) != -1)
            return;
    }
    ...
    case Friendly:
     ...
     // No break. That is not an error. We must add point to go then enemy or nothing
    case Enemy:
        m_stops.append(QPair<uint, uint>(point.first + vec_x, point.second + vec_y));
        checkExisting(QPair<uint, uint>(point.first + vec_x, point.second + vec_y),
                      vec_x, vec_y);
...
{
    while(point.first > 0 && point.first < 9 &&
