...
void HistoryModel::setRecord(HistoryModel::Type type, QPair<QString, uint> &from,
...
void HistoryModel::goForward()
{
    ...
    if(exp.indexIn(m_data.at(m_pos)) != -1)
        emit changed(toType(exp.cap(1)), QPair<QString, uint>(exp.cap(2), exp.cap(3).toUInt()),
                     QPair<QString, uint>(exp.cap(4), exp.cap(5).toUInt()), toStatus(exp.cap(6)));
}
